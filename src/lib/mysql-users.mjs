...
export async function createUser(user, { env = process.env } = {}) {
  ...
  try {
    await connection.execute(
      "INSERT INTO users (id, name, email, password_hash, role, github_id) VALUES (?, ?, ?, ?, ?, ?)",
      [user.id, user.name || null, user.email || null, user.passwordHash || null, user.role || 'Student', user.githubId || null]
    );
